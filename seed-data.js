// Run this in your browser console to populate the app with sample data
...
const seedData = () => {
  ...
  localStorage.setItem('skilllink_products', JSON.stringify(SAMPLE_PRODUCTS));
  localStorage.setItem('skilllink_services', JSON.stringify(SAMPLE_SERVICES));
  localStorage.setItem('skilllink_requests', JSON.stringify(SAMPLE_REQUESTS));
  
  // Merge profiles with existing ones to avoid overwriting current user
  const existingProfiles = JSON.parse(localStorage.getItem('skilllink_profiles') || '[]');
  const mergedProfiles = [...existingProfiles];
  
  SAMPLE_PROFILES.forEach(profile => {
    if (!mergedProfiles.find(p => p.id === profile.id)) {
      mergedProfiles.push(profile);
    }
  });
  
  localStorage.setItem('skilllink_profiles', JSON.stringify(mergedProfiles));
  
  console.log('✅ Data seeded successfully!');
  console.log('Added products:', SAMPLE_PRODUCTS.length);
  console.log('Added services:', SAMPLE_SERVICES.length);
  console.log('Added requests:', SAMPLE_REQUESTS.length);
